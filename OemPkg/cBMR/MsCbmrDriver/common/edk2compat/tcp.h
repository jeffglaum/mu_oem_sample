#ifndef _EDK2COMPAT_TCP_H_
#define _EDK2COMPAT_TCP_H_

// MdePkg/Include/Protocol/Tcp6.h

/** @file
EFI TCPv6(Transmission Control Protocol version 6) Protocol Definition
The EFI TCPv6 Service Binding Protocol is used to locate EFI TCPv6 Protocol
drivers to create and destroy child of the driver to communicate with other host
using TCP protocol. The EFI TCPv6 Protocol provides services to send and receive
data stream.
...
///
/// EFI_TCP6_SERVICE_POINT is deprecated in the UEFI 2.4B and should not be used
/// any more. The definition in here is only present to provide backwards
/// compatability.
///
typedef struct {
    ///
...
///
/// EFI_TCP6_VARIABLE_DATA is deprecated in the UEFI 2.4B and should not be used
/// any more. The definition in here is only present to provide backwards
/// compatability.
///
typedef struct {
    ...
    UINT32 ServiceCount; ///< The number of address/port pairs following this data
                         ///< structure.
    EFI_TCP6_SERVICE_POINT
    Services[1]; ///< List of address/port pairs that are currently in use.
...
typedef struct {
    ...
    /// EFI_CONNECTION_RESET:     The connect fails because the connection is
    /// reset
    ///                           either by instance itself or the communication
    ///                           peer.
    /// EFI_CONNECTION_REFUSED:   The receiving or transmission operation fails
    /// because this
    ...
    ///                           because an ICMP protocol unreachable error is
    ///                           received.
    /// EFI_PORT_UNREACHABLE:     The connection establishment
    ///                           timer times out and an ICMP port unreachable
    ///                           error is received.
    /// EFI_ICMP_ERROR:           The connection establishment timer times
    ///                           out and some other ICMP error is received.
    /// EFI_DEVICE_ERROR:         An unexpected system or network error occurred.
    /// EFI_SECURITY_VIOLATION:   The active open was failed because of IPSec
    /// policy check. EFI_NO_MEDIA:             There was a media error.
    ///
...
typedef struct {
    ...
    ///                         connection to this instance. A new TCP instance
    ///                         has also been created for the connection.
    /// EFI_CONNECTION_RESET:   The accept fails because the connection is reset
    /// either
    ///                         by instance itself or communication peer.
    /// EFI_ABORTED:            The accept request has been aborted.
    /// EFI_SECURITY_VIOLATION: The accept operation was failed because of IPSec
    /// policy check.
    ///
    EFI_TCP6_COMPLETION_TOKEN CompletionToken;
...
/// The EFI TCPv6 Protocol user must fill this data structure before sending a
/// packet. The packet may contain multiple buffers in non-continuous memory
/// locations.
///
typedef struct {
    ///
...
typedef struct {
    ...
    /// EFI_CONNECTION_FIN:       The receiving operation fails because the
    /// communication peer
    ///                           has closed the connection and there is no more
    ///                           data in the receive buffer of the instance.
    /// EFI_CONNECTION_RESET:     The receiving or transmission operation fails
    ///                           because this connection is reset either by
    ///                           instance itself or the communication peer.
    ...
    ///                           because an ICMP network unreachable error is
    ///                           received.
    ...
    ///                           operation was failed because of IPSec policy
    ///                           check
...
typedef struct {
    ...
    /// EFI_ABORTED:            User called configure with NULL without close
    /// stopping. EFI_SECURITY_VIOLATION: The close operation was failed because
    /// of IPSec policy check.
    ///
    EFI_TCP6_COMPLETION_TOKEN CompletionToken;
    ///
...
TCPv6 Protocol instance into user-supplied buffers. This function can also be
used to retrieve the operational setting of underlying drivers such as IPv6,
MNP, or SNP.
...
@param[out] SnpModeData       The buffer in which the current SNP mode data used
...
/**
Initialize or brutally reset the operational parameters for this EFI TCPv6
instance.

The Configure() function does the following:
- Initialize this TCP instance, i.e., initialize the communication end settings
and specify active open or passive open for an instance.
- Reset this TCP instance brutally, i.e., cancel all pending asynchronous
tokens, flush transmission and receiving buffer directly without informing the
communication peer.

No other TCPv6 Protocol operation except Poll() can be executed by this instance
until it is configured properly. For an active TCP instance, after a proper
configuration it may call Connect() to initiates the three-way handshake. For a
passive TCP instance, its state will transit to Tcp6StateListen after
configuration, and Accept() may be called to listen the incoming TCP connection
requests. If Tcp6ConfigData is set to NULL, the instance is reset. Resetting
process will be done brutally, the state machine will be set to Tcp6StateClosed
directly, the receive queue and transmit queue will be flushed, and no traffic
is allowed through this instance.

@param[in] This               Pointer to the EFI_TCP6_PROTOCOL instance.
@param[in] Tcp6ConfigData     Pointer to the configure data to configure the
instance. If Tcp6ConfigData is set to NULL, the instance is reset.

@retval EFI_SUCCESS           The operational settings are set, changed, or
reset successfully.
@retval EFI_NO_MAPPING        The underlying IPv6 driver was responsible for
choosing a source address for this instance, but no source address was available
for use.
...
instance properly.
@retval EFI_ACCESS_DENIED     Configuring TCP instance when it is configured
without calling Configure() with NULL to reset it.
...
**/
typedef EFI_STATUS(EFIAPI* EFI_TCP6_CONFIGURE)(IN EFI_TCP6_PROTOCOL* This,
...
configured in current TCP instance if it is configured active. If the connection
succeeds or fails due to any error, the ConnectionToken->CompletionToken.Event
will be signaled and ConnectionToken->CompletionToken.Status will be updated
accordingly. This function can only be called for the TCP instance in
Tcp6StateClosed state. The instance will transfer into Tcp6StateSynSent if the
function returns EFI_SUCCESS. If TCP three-way handshake succeeds, its state
will become Tcp6StateEstablished, otherwise, the state will return to
Tcp6StateClosed.
...
and the state of this TCP instance has been changed to Tcp6StateSynSent.
@retval EFI_NOT_STARTED        This EFI TCPv6 Protocol instance has not been
configured.
...
**/
typedef EFI_STATUS(EFIAPI* EFI_TCP6_CONNECT)(IN EFI_TCP6_PROTOCOL* This,
                                             IN EFI_TCP6_CONNECTION_TOKEN* ConnectionToken);

/**
Listen on the passive instance to accept an incoming connection request. This is
...
created instance is configured by inheriting the passive instance's
configuration and is ready for use upon return. The new instance is in the
Tcp6StateEstablished state.
...
This function only can be called when current TCP instance is in Tcp6StateListen
state.
...
operation.
@retval EFI_DEVICE_ERROR       Any unexpected and not belonged to above category
error.

**/
typedef EFI_STATUS(EFIAPI* EFI_TCP6_ACCEPT)(IN EFI_TCP6_PROTOCOL* This,
                                            IN EFI_TCP6_LISTEN_TOKEN* ListenToken);

/**
Queues outgoing data into the transmit queue.

The Transmit() function queues a sending request to this TCP instance along with
the user data. The status of the token is updated and the event in the token
...
@retval EFI_ACCESS_DENIED       One or more of the following conditions are
TRUE:
...
**/
typedef EFI_STATUS(EFIAPI* EFI_TCP6_TRANSMIT)(IN EFI_TCP6_PROTOCOL* This,
...
Token->CompletionToken.Event and the FragmentBuffer used to receive data. The
caller also must fill the DataLength which represents the whole length of all
FragmentBuffer. When the receive operation completes, the EFI TCPv6 Protocol
driver updates the Token->CompletionToken.Status and Token->Packet.RxData fields
and the Token->CompletionToken.Event is signaled. If got data the data and its
length will be copied into the FragmentTable, at the same time the full length
of received data will be recorded in the DataLength fields. Providing a proper
notification function and context for the event will enable the user to receive
the notification and receiving status. That notification function is guaranteed
...
due to a lack of system resources (usually memory).
@retval EFI_DEVICE_ERROR       An unexpected system or network error occurred.
...
and there is no any buffered data in the receive buffer of this instance
@retval EFI_NOT_READY          The receive request could not be queued because
the receive queue is full.

**/
typedef EFI_STATUS(EFIAPI* EFI_TCP6_RECEIVE)(IN EFI_TCP6_PROTOCOL* This,
                                             IN EFI_TCP6_IO_TOKEN* Token);

/**
Disconnecting a TCP connection gracefully or reset a TCP connection. This
function is a nonblocking operation.

Initiate an asynchronous close token to TCP driver. After Close() is called, any
buffered transmission data will be sent by TCP driver and the current instance
will have a graceful close working flow described as RFC 793 if AbortOnClose is
set to FALSE, otherwise, a rest packet will be sent by TCP driver to fast
disconnect this connection. When the close operation completes successfully the
TCP instance is in Tcp6StateClosed state, all pending asynchronous operations
are signaled and any buffers used for TCP network traffic are flushed.
...
**/
typedef EFI_STATUS(EFIAPI* EFI_TCP6_CLOSE)(IN EFI_TCP6_PROTOCOL* This,
...
Token->Event is signaled.
@retval EFI_INVALID_PARAMETER  This is NULL.
@retval EFI_NOT_STARTED        This instance hasn't been configured.
@retval EFI_NOT_FOUND          The asynchronous I/O request isn't found in the
transmission or receive queue. It has either completed or wasn't issued by
Transmit() and Receive().
@retval EFI_UNSUPPORTED        The implementation does not support this
function.

**/
typedef EFI_STATUS(EFIAPI* EFI_TCP6_CANCEL)(IN EFI_TCP6_PROTOCOL* This,
...
// Declared in MdePkg/Include/Protocol/Tcp4.h

/** @file
EFI TCPv4(Transmission Control Protocol version 4) Protocol Definition
The EFI TCPv4 Service Binding Protocol is used to locate EFI TCPv4 Protocol
drivers to create and destroy child of the driver to communicate with other host
using TCP protocol. The EFI TCPv4 Protocol provides services to send and receive
data stream.
