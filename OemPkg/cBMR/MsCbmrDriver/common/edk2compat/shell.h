...
**/
typedef EFI_STATUS(EFIAPI* EFI_SHELL_CREATE_FILE)(IN CONST CHAR16* FileName,
...
**/
typedef EFI_STATUS(EFIAPI* EFI_SHELL_GET_DEVICE_NAME)(IN EFI_HANDLE DeviceHandle,
...
**/
typedef CONST
    EFI_DEVICE_PATH_PROTOCOL*(EFIAPI* EFI_SHELL_GET_DEVICE_PATH_FROM_MAP)(IN CONST CHAR16* Mapping);
...
**/
typedef CONST CHAR16*(EFIAPI* EFI_SHELL_GET_ENV_EX)(IN CONST CHAR16* Name,
...
**/
typedef EFI_STATUS(EFIAPI* EFI_SHELL_GET_FILE_POSITION)(IN SHELL_FILE_HANDLE FileHandle,
                                                        OUT UINT64* Position);
...
**/
typedef EFI_STATUS(EFIAPI* EFI_SHELL_GET_FILE_SIZE)(IN SHELL_FILE_HANDLE FileHandle,
                                                    OUT UINT64* Size);
...
**/
typedef EFI_STATUS(EFIAPI* EFI_SHELL_GET_GUID_FROM_NAME)(IN CONST CHAR16* GuidName,
                                                         OUT EFI_GUID* Guid);
...
**/
typedef EFI_STATUS(EFIAPI* EFI_SHELL_GET_GUID_NAME)(IN CONST EFI_GUID* Guid,
...
**/
typedef EFI_STATUS(EFIAPI* EFI_SHELL_GET_HELP_TEXT)(IN CONST CHAR16* Command,
...
**/
typedef EFI_STATUS(EFIAPI* EFI_SHELL_OPEN_FILE_BY_NAME)(IN CONST CHAR16* FileName,
                                                        OUT SHELL_FILE_HANDLE* FileHandle,
                                                        IN UINT64 OpenMode);
...
**/
typedef EFI_STATUS(EFIAPI* EFI_SHELL_OPEN_FILE_LIST)(IN CHAR16* Path,
                                                     IN UINT64 OpenMode,
...
/**
  ...
  @param[out] FileHandle        On exit, points to the file handle corresponding to the root
...
/**
  ...
  @param[in] ReadSize            On input, the size of Buffer, in bytes. On output, the amount of
data read.
...
**/
typedef EFI_STATUS(EFIAPI* EFI_SHELL_REGISTER_GUID_NAME)(IN CONST EFI_GUID* Guid,
...
/**
  ...
  @param[in] Alias              Points to the NULL-terminated alias for the shell command. If this
is NULL, and Command refers to an alias, that alias will be deleted.
  @param[in] Replace            If TRUE and the alias already exists, then the existing alias will
be replaced. If FALSE and the alias already exists, then the existing alias is unchanged and
                                EFI_ACCESS_DENIED is returned.
  @param[in] Volatile           if TRUE the Alias being set will be stored in a volatile fashion. if
FALSE the Alias being set will be stored in a non-volatile fashion.

  @retval EFI_SUCCESS           Alias created or deleted successfully.
  @retval EFI_ACCESS_DENIED     The alias is a built-in alias or already existed and Replace was set
to FALSE.
**/
typedef EFI_STATUS(EFIAPI* EFI_SHELL_SET_ALIAS)(IN CONST CHAR16* Command,
...
**/
typedef CONST CHAR16*(EFIAPI* EFI_SHELL_GET_ALIAS)(IN CONST CHAR16* Alias,
...
/**
  ...
  @param[in] FileSystem         A pointer to the file system's mapped name. If NULL, then the
current working directory is changed.
  @param[in] Dir                Points to the NULL-terminated directory on the device specified by
FileSystem.

  @retval NULL                  Current directory does not exist.
  @return                       The current directory.
**/
typedef EFI_STATUS(EFIAPI* EFI_SHELL_SET_CUR_DIR)(IN CONST CHAR16* FileSystem OPTIONAL,
                                                  IN CONST CHAR16* Dir);
...
/**
  ...
  @param[in] Value              Points to the NULL-terminated environment variable value. If the
value is an empty string then the environment variable is deleted.
  @param[in] Volatile           Indicates whether the variable is non-volatile (FALSE) or volatile
(TRUE).

  @retval EFI_SUCCESS           The environment variable was successfully updated.
**/
typedef EFI_STATUS(EFIAPI* EFI_SHELL_SET_ENV)(IN CONST CHAR16* Name,
...
**/
typedef EFI_STATUS(EFIAPI* EFI_SHELL_SET_FILE_INFO)(IN SHELL_FILE_HANDLE FileHandle,
...
**/
typedef EFI_STATUS(EFIAPI* EFI_SHELL_SET_FILE_POSITION)(IN SHELL_FILE_HANDLE FileHandle,
                                                        IN UINT64 Position);

/**
  This function creates a mapping for a device path.

  @param[in] DevicePath         Points to the device path. If this is NULL and Mapping points to a
valid mapping, then the mapping will be deleted.
...
**/
typedef EFI_STATUS(EFIAPI* EFI_SHELL_SET_MAP)(IN CONST EFI_DEVICE_PATH_PROTOCOL* DevicePath,
